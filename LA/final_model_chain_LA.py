...
#run parameters
total_steps = 10
pop_tol = .01  #may need to adjust for enacted maps
run_name = 'test' 
run_type = 'free' 
model_mode = 'statewide'  #'district', equal, statewide
start_map = 'CD' #SEND, HDIST, CD or 'new_seed'
ensemble_inclusion = False
enacted_black = 1 #1 cong, 11 senate
num_districts = 6 #105 state house, 39 senate, 6 Cong
...
#make elections data structures
...
#state_gdf["SEND"] = state_gdf["SEND"].astype(int) -1
#state_gdf["HDIST"] = state_gdf["HDIST"].astype(int) -1
#state_gdf["CD"] = state_gdf["CD"].astype(int) -1
...
def final_elec_model(partition):  
    ...
    black_effective = [j for j,k in optimize_dict.values()]
    ...
    return final_state_prob_dict, final_equal_prob_dict, final_dist_prob_dict, \
            total_black_final, optimize_dict
...
map_metric = pd.DataFrame(columns = ["BO", "Cut Edges", "County Splits"], index = list(range(store_interval)))
...
for step in chain:
    ...
    total_black_final, optimize_dict = step["final_elec_model"]
    map_metric.loc[step_Num] = [total_black_final, step["num_cut_edges"], step["num_county_splits"]]
